...
class CEDenoiser(nn.Module):
    def __init__(
        ...
        margin,
        eps,
        use_missing_aware_conditioning=False,
        use_missing_aware_film_conditioning=False,
        availability_dim=4,
    ):
        ...
        self.use_missing_aware_conditioning = use_missing_aware_conditioning
        self.use_missing_aware_film_conditioning = use_missing_aware_film_conditioning
        self.availability_proj = nn.Linear(availability_dim, embedding_dim)
        self.availability_gamma = nn.Linear(availability_dim, embedding_dim)
        self.availability_beta = nn.Linear(availability_dim, embedding_dim)
        nn.init.zeros_(self.availability_gamma.weight)
        nn.init.zeros_(self.availability_gamma.bias)
        nn.init.zeros_(self.availability_beta.weight)
        nn.init.zeros_(self.availability_beta.bias)

    def forward(self, x_t, t, x_ht, x_r, mode, availability=None):
        c = self.scoring_module(x_ht, x_r, mode)
        if self.use_missing_aware_conditioning and availability is not None:
            c = c + self.availability_proj(availability.float())
        if self.use_missing_aware_film_conditioning and availability is not None:
            availability = availability.float()
            gamma = self.availability_gamma(availability)
            beta = self.availability_beta(availability)
            c = (1.0 + gamma) * c + beta
...
class DiffHEG(nn.Module):
    def __init__(
        ...
        self.denoiser = CEDenoiser(
            embedding_dim,
            dim_r,
            margin,
            eps,
            use_missing_aware_conditioning=use_missing_aware_conditioning,
            use_missing_aware_film_conditioning=use_missing_aware_film_conditioning,
    ...
    def forward(self, x_h, x_r, x_t, availability=None):
        ...
        t_noise_pred = self.denoiser(x_t_noisy, t_tensor, x_h, x_r, 'tail', availability=availability)
        
        x_h_noisy = self.q_sample(x_h, t_tensor, epsilon)
        h_noise_pred = self.denoiser(x_h_noisy, t_tensor, x_t, x_r, 'head', availability=availability)
    ...
    def p_sample(self, x_noisy, t, x_ht, x_r, mode, availability=None):
        beta_t = self.betas[t - 1]
        alpha_t = self.alphas[t - 1]
        alpha_bar_t = self.alpha_bars[t - 1]

        noise_pred = self.denoiser(x_noisy, t, x_ht, x_r, mode, availability=availability)
    ...
    @torch.no_grad()
    def sample(self, x_h, x_r, x_t, availability=None):
        ...
        for t in range(self.T, 0, -1):
            ...
            x_t_noisy = self.p_sample(x_t_noisy, t_tensor, x_h, x_r, 'tail', availability=availability)
            x_h_noisy = self.p_sample(x_t_noisy, t_tensor, x_t, x_r, 'head', availability=availability)
