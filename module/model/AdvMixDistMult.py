...
class AdvMixDistMult(Model):
    ...
    def _get_has_text_mask(self, batch_entities, device):
        if self.has_text is None:
            return None
        entity_ids = batch_entities.detach().view(-1).cpu()
        return self.has_text.index_select(0, entity_ids).to(device=device, dtype=torch.float32).unsqueeze(-1)
    ...
    def mm_negative_score(
        ...
        score_h_struct = self._calc(neg_h, t, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_h_img = self._calc(neg_hv, t, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_h_text = self._calc(neg_ht, t, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)

        score_t_struct = self._calc(h, neg_t, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_t_img = self._calc(h, neg_tv, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_t_text = self._calc(h, neg_tt, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)

        score_all_struct = self._calc(neg_h, neg_t, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_all_img = self._calc(neg_hv, neg_tv, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)
        score_all_text = self._calc(neg_ht, neg_tt, r, mode).view(-1, batch_h.shape[0]).permute(1, 0)

        if not self.use_missing_mask or self.has_text is None:
            self.last_mask_debug = None
            score_h = (score_h_struct + score_h_img + score_h_text) / 3
            score_t = (score_t_struct + score_t_img + score_t_text) / 3
            score_all = (score_all_struct + score_all_img + score_all_text) / 3
            return [score_h, score_t, score_all]

        head_text_mask = self._get_has_text_mask(batch_h, score_h_struct.device)
        tail_text_mask = self._get_has_text_mask(batch_t, score_t_struct.device)
        pair_text_mask = head_text_mask * tail_text_mask
        self.last_mask_debug = {
            "head_missing_text_count": int((head_text_mask == 0).sum().item()),
            "tail_missing_text_count": int((tail_text_mask == 0).sum().item()),
            "score_all_text_masked_count": int((pair_text_mask == 0).sum().item()),
        }

        score_h = (score_h_struct + score_h_img + head_text_mask * score_h_text) / (2.0 + head_text_mask)
        score_t = (score_t_struct + score_t_img + tail_text_mask * score_t_text) / (2.0 + tail_text_mask)
        score_all = (score_all_struct + score_all_img + pair_text_mask * score_all_text) / (2.0 + pair_text_mask)
        return [score_h, score_t, score_all]
